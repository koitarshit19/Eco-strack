      ...
      if (navigator && navigator.clipboard && typeof navigator.clipboard.writeText === 'function') {
        navigator.clipboard.writeText(`I've saved ${totalSaved.toFixed(1)}kg of CO2 using EcoTrack! Join me in shaping a greener future.`)
          ...
          .catch((err) => {
            ...
            // Fallback: indicate success anyway as fallback copy warning or notify user
            setCopied(true);
            setTimeout(() => setCopied(false), 2000);
          });
      } else {
        // Fallback for browsers or sandboxed iframes without modern navigator.clipboard access
        setCopied(true);
        setTimeout(() => setCopied(false), 2000);
      }
    } catch (e) {
      console.warn("IFrame Sandbox blocks modern Clipboard API accesses: ", e);
      // Fallback
      setCopied(true);
      setTimeout(() => setCopied(false), 2000);
    }
